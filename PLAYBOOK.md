...
### The demo IS the book (one truth, mechanized)

Ruled by the owner at 0.0.52: the demo and the book of examples contain
exactly the same examples, forever. Enforcement is structural, not
disciplinary: packages/demo.cz is GENERATED by tools/gen_demo.py from
BOOK.md — chapters become sections, **Problem** transcripts become
replayable steps (demo() menu, demo(k) section, Enter advances,
demo(0) plays all) — and the generator's --check lives in make test.
Editing the book regenerates the demo; editing the demo is impossible.
Two exclusions by rule, stated in the generator: workspace-surgery
problems (clear/keep) would destroy the demo's own session, so they
stay book-only. State the demo maintains that eval() does not: ans
after each echoed value, and format(6) at each problem boundary,
because book transcripts are fresh sessions and the demo is one long
